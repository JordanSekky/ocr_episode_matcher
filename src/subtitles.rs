...
pub fn select_best_track(tracks: &[SubtitleTrack]) -> Option<&SubtitleTrack> {
    ...
    let english_tracks: Vec<&SubtitleTrack> = tracks.iter()
...
pub fn extract_and_display_subtitles(path: &Path, track: &SubtitleTrack) -> Result<()> {
    ...
    if track.is_text {
        ...
        let pager_stdin = pager_cmd.stdin.take().context("Failed to open pager stdin")?;

        let mut ffmpeg = Command::new("ffmpeg")
            .args(&[
                "-i", path.to_str().unwrap(),
                "-map", &format!("0:{}", track.index),
                "-f", "srt",
                "-",
            ])
...
fn extract_and_ocr_pgs(path: &Path, track: &SubtitleTrack, pager: &str) -> Result<()> {
    ...
    let output_pattern_str = temp_path_buf.join("sub_%04d.png").to_string_lossy().to_string();
    ...
    let mut pager_stdin = pager_cmd.stdin.take().context("Failed to open pager stdin")?;
    ...
    thread::spawn(move || -> Result<()> {
        ...
        let mut api = crate::ocr::create_ocr_engine()?;
        ...
        loop {
            ...
            if next_file.exists() {
                ...
                if ffmpeg_finished.load(Ordering::Acquire) {
                     ...
                     if !ocr_path_buf.join(format!("sub_{:04}.png", processed_count + 1)).exists() {
                         break;
                     }
                } else {
